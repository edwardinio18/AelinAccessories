<?php
	...
	if (isset($_GET["convoId"]) && isset($_GET["orderId"]) && isset($_GET["offset"]) && isset($_SESSION["id"])) {
		...
		$query1 = "SELECT * FROM conversations WHERE convoId = ? AND toUserIdConvo = ? AND orderIdConvo = ? ORDER BY updateDateAndTimestamp DESC;";
				...
				if ($result2->num_rows >= 1) {
					...
					while ($row2 = $result2->fetch_assoc()) {
						...
						if ($row2["fromUserId"] == $userId) {
							...
							$deleteMessageImage = '<img src="images/bin.png" class="delete_chat_bin_image_admin" data-messageid="' . $row2["id"] . '" data-messagecontent="' . $messageContent . '" data-orderid="' . $numericalOrderId . '" data-otherorderid="' . $orderId . '" data-convoid="' . $row1["convoId"] . '" />';
