<?php
	...
	if (isset($_GET["convoId"]) && isset($_GET["orderId"]) && isset($_SESSION["id"])) {
		...
		$query1 = "SELECT * FROM conversations WHERE convoId = ? AND (fromUserIdConvo = ? OR toUserIdConvo = ?) AND orderIdConvo = ? ORDER BY updateDateAndTimestamp DESC;";
		$stmt1 = $db->prepare($query1);
		$stmt1->bind_param("iiii", $convoId, $userId, $userId, $numericalOrderId);
		$stmt1->execute();
		...
		if ($result1->num_rows >= 1) {
			...
			if ($row1 = $result1->fetch_assoc()) {
				...
				$query2 = "SELECT * FROM messages WHERE (fromUserId = ? OR toUserId = ?) AND orderId = ? ORDER BY id DESC LIMIT 7;";
				$stmt2 = $db->prepare($query2);
				$stmt2->bind_param("iii", $userId, $userId, $numericalOrderId);
				$stmt2->execute();
				...
				if ($result2->num_rows >= 1) {
					...
					while ($row2 = $result2->fetch_assoc()) {
						...
						if ($row2["fromUserId"] == $userId) {
							...
							if ($_SESSION["id"] == $ADMIN_USER_ID) {
								...
								$deleteMessageImage = '<img src="images/bin.png" class="delete_chat_bin_image_admin" data-messageid="' . $row2["id"] . '" data-messagecontent="' . $messageContent . '" data-orderid="' . $numericalOrderId . '" data-otherorderid="' . $orderId . '" data-touserid="' . $row2["toUserId"] . '" data-convoid="' . $convoId . '" />';

							} else {

								$deleteMessageImage = '<img src="images/bin.png" class="delete_chat_bin_image" data-messageid="' . $row2["id"] . '" data-messagecontent="' . $messageContent . '" data-orderid="' . $numericalOrderId . '" data-otherorderid="' . $orderId . '" data-touserid="' . $row2["toUserId"] . '" data-convoid="' . $convoId . '" />';
					...
					$output = '<div class="select_and_open_conversation_div">

								<p class="no_messages_yet">No messages have been sent in this conversation yet!</p>
