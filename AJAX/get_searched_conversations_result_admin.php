<?php
	...
	if (isset($_GET["orderIdCurVal"]) && isset($_SESSION["id"])) {
		...
		$query1 = "SELECT * FROM conversations JOIN orders ON orders.id = conversations.orderIdConvo JOIN products ON products.id = orders.productId WHERE conversations.toUserIdConvo = ? AND uniqueOrderId LIKE ?;";
		...
		if ($result1->num_rows >= 1) {
			...
			while ($row1 = $result1->fetch_assoc()) {
				...
				$output .= '<div class="each_conversation_main_outer_container_div_admin each_conversation_main_outer_container_div_admin_' . $row1["orderIdConvo"] . '" id="' . $uniqueOrderId . '" data-convoid="' . $conversationId . '" data-orderid="' . $row1["orderIdConvo"] . '" data-imagesource="' . $productImagePath . '" data-touserid="' . $userIdConvo . '">
