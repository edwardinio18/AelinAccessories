<?php
	...
	$query1 = "SELECT * FROM conversations JOIN orders ON orders.id = conversations.orderIdConvo JOIN products ON products.id = orders.productId WHERE conversations.toUserIdConvo = ? ORDER BY updateDateAndTimestamp DESC LIMIT 1;";
		...
		while ($row1 = $result1->fetch_assoc()) {
			...
			if ($result2->num_rows >= 1) {
				...
				if ($row2 = $result2->fetch_assoc()) {
					...
					$output .= '<div class="each_conversation_main_outer_container_div_admin each_conversation_main_outer_container_div_admin_' . $row1["orderIdConvo"] . ' active_conversation" id="' . $uniqueOrderId . '" data-convoid="' . $conversationId . '" data-orderid="' . $row1["orderIdConvo"] . '" data-imagesource="' . $productImagePath . '" data-touserid="' . $userIdConvo . '">
	...
	$query5 = "SELECT * FROM conversations WHERE toUserIdConvo = ?;";
	...
	$query3 = "SELECT * FROM conversations JOIN orders ON orders.id = conversations.orderIdConvo JOIN products ON products.id = orders.productId WHERE conversations.toUserIdConvo = ? ORDER BY updateDateAndTimestamp DESC LIMIT " . $rowsLimit . " OFFSET 1;";
		...
		while ($row3 = $result3->fetch_assoc()) {
			...
			if ($result4->num_rows >= 1) {
				...
				if ($row4 = $result4->fetch_assoc()) {
					...
					$userIdConvo = $row3["fromUserIdConvo"];

					$output .= '<div class="each_conversation_main_outer_container_div_admin each_conversation_main_outer_container_div_admin_' . $row3["orderIdConvo"] . '" id="' . $uniqueOrderId . '" data-convoid="' . $conversationId . '" data-orderid="' . $row3["orderIdConvo"] . '" data-imagesource="' . $productImagePath . '" data-touserid="' . $userIdConvo . '">
